 ...
 * architecture promises - the wizard may depend on the BackendCreateModule
 * contract and the registry, but never on a concrete backend module.
 */
...
void test("newProjectWizard.ts contains no framework-id string-literal dispatch", () => {
  ...
  assert.equal(/backendModule\.id\s*===/.test(wizardSource), false, "the wizard must not branch on module.id");
});

void test("newProjectWizard.ts imports only the generic BackendCreateModule contract and registry from project/create/", () => {
  const createImports = [...wizardSource.matchAll(/from\s+["']([^"']*\/project\/create\/[^"']*)["']/g)].map((match) => match[1]);
  for (const importPath of createImports) {
    assert.ok(
      /\/create\/(backendCreateModule|backendCreateModules|projectStepsComposition)$/.test(importPath ?? ""),
...
/**
 * The wizard treats backendPlan.confirmationSummary as an opaque

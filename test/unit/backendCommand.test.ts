...
import { buildDjangoRunServerCommand } from "../../src/execution/backendCommand";
...
void test("builds the Django runserver command with the detected python, manage.py, host and port", () => {
  const command = buildDjangoRunServerCommand(python(), backend(), "127.0.0.1", 8000);
...
void test("never substitutes a different host than the one configured", () => {
  const command = buildDjangoRunServerCommand(python(), backend(), "0.0.0.0", 8080);
